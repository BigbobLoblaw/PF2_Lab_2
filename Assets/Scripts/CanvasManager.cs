...
public class CanvasManager : MonoBehaviour
{
    ...
    private void Update()
    {
        if (objects.Count > 0 && !isSpawning)
        {
            ...
            {
                StartCoroutine(QueueTime(objectSpawn.object1_Prefab, 2));
            }

            if (objects.Peek() == objectSpawn.object2_Prefab)
            {
                StartCoroutine(QueueTime(objectSpawn.object2_Prefab, 4));
            }

            if (objects.Peek() == objectSpawn.object3_Prefab)
            {
                StartCoroutine(QueueTime(objectSpawn.object3_Prefab, 6));
            }
        }
    }
}
